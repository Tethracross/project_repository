...
void Map::setMap() {
    int randRoom = randomNum(8); // 8 possible rooms

    switch(randRoom) { // switches for each room case
    // 5 rooms per map
    case 1:
        //  Room Layout
        addRoom(2, 10); 
...
Return: boolean (bool)
 */
bool Map::move(char direction, int arr[2])
{
    ...
    arr[0] = player_position_[0];
    arr[1] = player_position_[1];
    return true;
}
